...
class NLayerDiscriminatorSN(nn.Module):
    def __init__(self, input_nc, ndf=64, norm_layer=nn.BatchNorm2d, use_sigmoid=False):
        ...
        self.sequence2 = nn.Sequential(
            spectral_norm(nn.Conv2d(ndf, ndf * 2, kernel_size=kw, stride=2, padding=padw, bias=use_bias)),
            norm_layer(ndf * 2),
            nn.LeakyReLU(0.2, True)
        )

        self.sequence3 = nn.Sequential(
            spectral_norm(nn.Conv2d(ndf * 2, ndf * 4, kernel_size=kw, stride=2, padding=padw, bias=use_bias)),
            norm_layer(ndf * 4),
            nn.LeakyReLU(0.2, True)
        )

        final_sequence = list()
        final_sequence += [nn.Conv2d(ndf * 4, 1, kernel_size=kw, stride=2, padding=padw)] + [nn.Dropout(0.5)]
        if use_sigmoid:
            final_sequence += [nn.Sigmoid()]

        self.final_sequence = nn.Sequential(*final_sequence)

    def forward(self, input):

        f_1 = self.sequence1(input)
        f_2 = self.sequence2(f_1)
        f_3 = self.sequence3(f_2)
        output = self.final_sequence(f_3)

        return f_1, f_2, f_3, output


class NLayerDiscriminatorSNseq(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layer=4, norm_layer=nn.BatchNorm2d, use_sigmoid=False):
        super(NLayerDiscriminatorSNseq, self).__init__()
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
        ...
        padw = 1
        self.sequence1 = nn.Sequential(
            spectral_norm(nn.Conv2d(input_nc, ndf, kernel_size=kw, stride=2, padding=padw, bias=use_bias)),
            nn.LeakyReLU(0.2, True),
        )
        ...
        for i in range(1, n_layer):
            seq = nn.Sequential(
                nn.Conv2d(ndf * (2**(i-1)), ndf * (2**i), kernel_size=kw, stride=2, padding=padw, bias=use_bias),
                norm_layer(ndf * (2**i)),
                nn.LeakyReLU(0.2, True)
            )
            self.seq_reg += [seq]
        self.sequences = nn.ModuleList(self.seq_reg)

        final_sequence = list()
        final_sequence += [nn.Conv2d(ndf * (2**i), 1, kernel_size=kw, stride=2, padding=padw)] + [nn.Dropout(0.5)]
        if use_sigmoid:
            final_sequence += [nn.Sigmoid()]
    ...
    def forward(self, input):
        ...
        feat_list = list()
        f = self.sequence1(input)
        feat_list.append(f)
        for i in range(0, self.n_layer - 1):
            f_ = self.sequences[i](f)
            feat_list.append(f_)
            f = f_
        output = self.final_sequence(f)
...
class MultiScaleDiscriminator(nn.Module):
    ...
    def make_net(self):
        ...
        # Entry block
        net += [nn.utils.spectral_norm(nn.Conv2d(1, base_channels, kernel_size=3, stride=1)),
