...
class ModelTrainerIMGgan:
    ...
    def __init__(self, args, modelG_DtoF, modelG_FtoD, modelD_Full, modelD_Down, train_loader,
        ...
        self.feat_lambda = torch.tensor(args.feat_lambda, dtype=torch.float32, device=args.device)
        self.n_layer = args.n_layer  # the numer of blocks of seqD
    ...
    def _train_epoch(self, epoch):
        ...
        for step, data in data_loader:
            ...
            img_full, outputs_full_cfc, img_down, outputs_down_fcf, fake_img_full, step_G_loss, step_metrics\
            ...
            with torch.no_grad():  # Update epoch loss and metrics
                if self.use_slice_metrics:
                    slice_metrics = self._get_slice_metrics(fake_img_full, img_full, self.batch_size)
                ...
                if self.display_interval and (step % self.display_interval == 0):
                    ...
                    ir_grid = make_grid_doublet(img_down_grid, img_recon_grid)
                    self.writer.add_image(f'DtoF/{step}', ir_grid, epoch, dataformats='HW')

                    # F D F
                    img_full_grid = normalize_im(img_full.squeeze())
                    img_fakedown_grid = normalize_im(outputs_down_fcf.squeeze())
                    img_fakefull_grid = normalize_im(fake_img_full.squeeze())
                    irt_grid = make_grid_triplet(img_full_grid, img_fakedown_grid, img_fakefull_grid)
                    self.writer.add_image(f'FDF/{step}', irt_grid, epoch, dataformats='HW')
    ...
    def _train_step_G(self, img_down, img_full, extra_params):
        ...
        cyclic_loss_cfc = self.losses['cyclic_loss'](img_down, fake_img_down)
        feat_loss_cfc, D_loss_cfc = self._calc_GANloss(self.modelD_Full, outputs_full_cfc, True)

        step_loss_cfc = cyclic_loss_cfc + self.feat_lambda * feat_loss_cfc + self.GAN_lambda * D_loss_cfc

        ## Full - Down - Full ##
        outputs_down_fcf = self.modelG_FtoD(img_full)
        fake_img_full = self.modelG_DtoF(outputs_down_fcf)

        cyclic_loss_fcf = self.losses['cyclic_loss'](img_full, fake_img_full)
        feat_loss_fcf, D_loss_fcf = self._calc_GANloss(self.modelD_Down, outputs_down_fcf, True)

        step_loss_fcf = cyclic_loss_fcf + self.feat_lambda * feat_loss_fcf + self.GAN_lambda2 * D_loss_fcf

        step_metrics = {'cyclic_loss_cfc': cyclic_loss_cfc, 'D_loss_cfc': D_loss_cfc,
                        'cyclic_loss_fcf': cyclic_loss_fcf, 'D_loss_fcf': D_loss_fcf}

        step_loss = step_loss_cfc + step_loss_fcf
        step_loss.backward()

        self.optimizerG_DtoF.step()
        self.optimizerG_FtoD.step()

        return img_full, outputs_full_cfc, img_down, outputs_down_fcf, fake_img_full, step_loss, step_metrics
    ...
    def _train_step_D_double(self, img_full, outputs_full_cfc, img_down, outputs_down_fcf):
        ...
        # D Full
        # Real
        feat_loss_D_real_full, loss_D_real_full = self._calc_GANloss(self.modelD_Full, img_full, True)
        # Fake
        outputs_full_cfc = self.fake_full_pool.query(outputs_full_cfc.detach())
        feat_loss_D_fake_full, loss_D_fake_full = self._calc_GANloss(self.modelD_Full, outputs_full_cfc, False)

        loss_D_Full = (feat_loss_D_real_full + feat_loss_D_fake_full) + self.GAN_lambda * (loss_D_real_full + loss_D_fake_full)
        loss_D_Full.backward()
        self.optimizerD_Full.step()

        # D Down
        # Real
        feat_loss_D_real_down, loss_D_real_down = self._calc_GANloss(self.modelD_Down, img_down, True)
        # Fake
        outputs_down_fcf = self.fake_down_pool.query(outputs_down_fcf.detach())
        feat_loss_D_fake_down, loss_D_fake_down = self._calc_GANloss(self.modelD_Down, outputs_down_fcf, True)

        loss_D_Down = (feat_loss_D_real_down + feat_loss_D_fake_down) + self.GAN_lambda * (loss_D_real_down + loss_D_fake_down)
    ...
    def _calc_GANloss(self, modelD, img, TF):
        if TF == True:
            feat, pred = modelD(img)
        else:
            feat, pred = modelD(img.detach())
        feat_loss = 0
        for i in range(self.n_layer):
            feat_loss += self.losses['GAN_loss'](feat[i], True)
        pred_loss = self.losses['GAN_loss'](pred, TF)
        return feat_loss, pred_loss
