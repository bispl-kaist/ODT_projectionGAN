...
from utils.train_utils import create_cycle_data_loaders_axial_single

from data.input_transforms import Prefetch2Device, PreProcessScale_Axial, PreProcessScale_Axial_nothing
...
from train.A_model_trainers.A_trainer_cycleGAN import ModelTrainerIMGgan
...
def train_img(args):
    ...
    # input_train_transform = PreProcessScale_Axial_nothing(args.device, use_seed=True, divisor=divisor, patch_size=args.patch_size)
    ...
    modelD_Full = NLayerDiscriminator(input_nc=data_chans, use_sigmoid=False).to(device)
    modelD_Down = NLayerDiscriminator(input_nc=data_chans, use_sigmoid=False).to(device)
...
if __name__ == '__main__':
    settings = dict(
        ...
        name='Your_name',  # Please do change this every time Harry
        data_root=None,
        log_root=None,
        ckpt_root=None,
        batch_size=1,
        ...
        display_images=100,  # Maximum number of images to save.
        num_workers=3,
        init_lr=1e-4,
        gpu=0,
        max_to_keep=10,
